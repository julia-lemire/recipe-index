...
- ✅ FilterSortGroup library: Generic, extractable filter/sort/group library in utils/filtersort/ with core interfaces (Filter<T>, Sort<T>, GroupBy<T,K>), FilterSortGroupManager for reactive state management, recipe-specific implementations (FavoriteFilter, TagFilter, SourceFilter, CookTimeFilter, ServingsFilter, HasPhotoFilter, TitleSort, DateCreatedSort, CookTimeSort, ServingsSort, FavoriteSort, SourceGrouping, FavoriteGrouping, TagGrouping, CookTimeGrouping, ServingsGrouping), reusable UI components (FilterChipRow, SortMenu), integrated into RecipeViewModel with search predicate matching title/ingredients/tags, 100% type-safe and zero app dependencies for easy extraction
