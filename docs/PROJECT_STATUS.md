...
- ✅ PDF parsing improvements: TextRecipeParser now joins continuation lines (joinInstructionLines for numbered steps, joinParagraphLines for prose), filters PDF page noise (URLs, page headers, page numbers) via isPdfPageNoise(), sourceTips field for preserving website notes/tips/substitutions separate from user notes, cleanIngredient() preserves quantities (only strips "1." numbering not "4 chicken")
