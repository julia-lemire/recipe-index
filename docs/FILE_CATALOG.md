...
- **TextRecipeParser.kt** - Smart pattern matching parser: detects ingredients/instructions/notes sections via regex, filters website noise (CTAs/footers/rating prompts) and PDF page noise (URLs/page headers), OCR noise cleaning (removes checkbox chars U/O/☐/□, fixes 0z→oz), breadcrumb navigation filtering in title extraction (skips lines with " > "), joins continuation lines for instructions (joinInstructionLines) and prose (joinParagraphLines), validates ingredient/instruction content, supports Unicode fractions (½, ¼, ¾, etc.) and OCR fractions with spaces (1 /2→½), parses time strings, extracts servings/servingSize (portion size)/sourceTips, cleans bullets/numbering, recovers misplaced ingredients from instructions when PDF multi-column extraction jumbles text order
