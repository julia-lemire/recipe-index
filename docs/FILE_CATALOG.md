...
- **TextRecipeParser.kt** - Smart pattern matching parser: detects ingredients/instructions/notes sections via regex with standalone header matching (`^ingredients?\s*:?\s*$`) and breadcrumb skipping (lines with " > "), filters website noise (CTAs/footers/rating prompts/subscribe/newsletter/"more X recipes") and PDF page noise (URLs/page headers), OCR noise cleaning (removes checkbox chars U/O/☐/□, fixes 0z→oz), enhanced title extraction with CTA pattern filtering (jump to/print/save/share/pin/rate/email), joins continuation lines for instructions (joinInstructionLines) and prose (joinParagraphLines), validates ingredient/instruction content, supports Unicode fractions (½, ¼, ¾, etc.) and OCR fractions with spaces (1 /2→½), parses time strings, extracts servings/servingSize (portion size including unitless fractions like "1/4")/sourceTips, cleans bullets/numbering, recovers misplaced ingredients from instructions when PDF multi-column extraction jumbles text order
