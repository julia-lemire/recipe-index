...
### Data - Entities
- **Recipe.kt** - Recipe entity with Room annotations: title, ingredients, instructions, servings, servingSize (portion size e.g. "1 ½ cups", "200g"), times, tags, source, photos, notes, sourceTips (tips/substitutions from source), behavioral flags (isFavorite, isTemplate)
...
- **GroceryListManager.kt** - Grocery list business logic: list/item CRUD operations, intelligent ingredient consolidation (removes modifiers, sums quantities), canned item parsing, recipe-to-list conversion with ingredient count tracking, meal plan-to-list conversion, pantry staples filtering with quantityExceedsThreshold() returning false for null quantity/unit items (assumes small amounts should be filtered) and true when quantities exceed configured thresholds (delegates to GroceryListDao, GroceryItemDao, RecipeDao, MealPlanDao, PantryStapleManager)
...
- **UrlRecipeParser.kt** - URL import controller: Orchestrates cascading supplementation strategy (Schema.org → HTML scraping → Open Graph) where each parser supplements missing fields without overwriting previous data, delegates to SchemaOrgRecipeParser/HtmlScraper/OpenGraphParser, handles HTTP fetching with Ktor client, enables saving partial recipes with metadata from multiple sources, comprehensive logging for each supplementation step
- **SchemaOrgRecipeParser.kt** - Schema.org JSON-LD parser: parse(Document) extracts structured recipe data from JSON-LD, handles Recipe/Article/BlogPosting types with embedded recipe fields, parses recipeCategory/recipeCuisine (keywords field excluded), extracts servingSize from nutrition field, extracts HTML category links via HtmlScraper, HowToStep/HowToSection instructions, ISO 8601 duration conversion, recursive nested JSON handling, cuisine extraction from titles, debug logging
...
- **TextRecipeParser.kt** - Smart pattern matching parser: detects title/ingredients/instructions/notes sections via regex with standalone header matching (`^ingredients?\s*:?\s*$`, `^title\s*:`) and breadcrumb skipping (lines with " > "), filters website noise (CTAs/footers/rating prompts/subscribe/newsletter/"more X recipes") and PDF page noise (URLs/page headers), OCR noise cleaning (removes checkbox chars U/O/☐/□, fixes 0z→oz), enhanced title extraction prioritizing explicit "Title:" header then CTA pattern filtering (jump to/print/save/share/pin/rate/email), joins continuation lines for instructions via joinInstructionLines() detecting numbered steps with various delimiters (`^\d+[.):\s]` matches "1.", "1)", "1:", "1 "), validates ingredient/instruction content, supports Unicode fractions (½, ¼, ¾, etc.) and OCR fractions with spaces (1 /2→½), parses time strings, extracts servings/servingSize (portion size including unitless fractions like "1/4")/sourceTips, cleans bullets/numbering, recovers misplaced ingredients from instructions when PDF multi-column extraction jumbles text order
...
### Data - Database
- **AppDatabase.kt** - Room database singleton: Recipe, MealPlan, GroceryList, GroceryItem, IngredientSubstitution, RecipeLog tables, version 9, fallbackToDestructiveMigration
...
### UI - Screens
- **HomeScreen.kt** - Landing page with recipe highlights: Quick Actions at top (Import with Download icon, View All buttons), This Week's Meal Plan section (shows dates and recipe count or empty state), Recent Recipes carousel (horizontal LazyRow with last 5 by createdAt), Favorites carousel (horizontal LazyRow with starred recipes up to 5), empty state when no recipes exist, uses HomeViewModel for data, full navigation integration
- **RecipeListScreen.kt** - Recipe browsing: LazyColumn with RecipeCards (photo from mediaPaths first IMAGE or photoPath fallback, title, servings/times, tags), toggle-able search bar (matching MealPlanningScreen), single FAB goes to tabbed add recipe screen, favorite toggle, delete from card menu, FilterChipRow and SortMenu integrated, empty state, Coil AsyncImage for photos (140dp), RecipeCard composable is internal for reuse in HomeScreen
...
- **AddEditRecipeScreen.kt** - Recipe add/edit form: Single screen with title, servings, times, ingredients, instructions, tags, notes, validation, auto-save on back, preserves existing mediaPaths when editing (prevents photo loss on recipe updates)
- **RecipeDetailScreen.kt** - Recipe detail view: Swipeable image carousel with HorizontalPager (240dp) showing all images from mediaPaths with page indicator dots when 2+ images and AddAPhoto overlay button for adding photos, simplified photo management via file picker (ActivityResultContracts.GetContent), savePhotoToStorage() compression, "Add Photo" placeholder card when no photos, text labels (Servings, Prep:, Cook:, Total:, Portion:) replacing emoji icons, info organized into two rows (Row 1: Servings with dropdown + Portion size, Row 2: Prep/Cook/Total times), unit conversion toggle (SwapHoriz icon) in ingredients header for switching between both units or settings preference, cook mode (checkable ingredients/instructions with Select All/Deselect All buttons in section headers, timer, keep awake, Quick Note button for adding notes without leaving cook mode, tags hidden for focus), long-press ingredient for substitution lookup, tabbed instruction sections, action buttons use TextButton for borderless appearance (Grocery/Meal Plan/Share/Favorite), edit/delete actions, Coil AsyncImage
...
- **GroceryListDetailScreen.kt** - Grocery list detail: Quick-entry text field at top, item checkboxes, item detail dialog showing source recipes, icon-over-text bottom actions (toggle Select All/Deselect, Delete, Recipes, Meal Plans) with clear destructive language, improved quantity formatting with common fractions (1/4, 1/2, 3/4) for small amounts and max 1 decimal place, "Delete Checked Items" dialog with explicit confirmation
...
### Settings
- **AppSettings.kt** - User preferences data class: liquidVolumePreference/dryVolumePreference/weightPreference enums (IMPERIAL/METRIC/BOTH), TemperatureUnit enum (FAHRENHEIT/CELSIUS), showPhotosInList, defaultServings
...
- **SettingsScreen.kt** - Settings UI: Granular unit preference sections (liquid volume, dry volume, weight) with IMPERIAL/METRIC/BOTH radio buttons, temperature radio buttons, display preferences switches, recipe defaults filter chips, reset button
...
- **RecipeTemplateHelper.kt** - Recipe template for manual text file import: RECIPE_TEMPLATE constant with explicit section headers (Title:, Servings:, etc.) and [bracket] placeholders for user replacement, simplified 3-line usage instructions, shareTemplate(context) launches Android share sheet for emailing/saving template, works with TextRecipeParser section detection including Title: header
