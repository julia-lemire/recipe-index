...
- **RecipeDetailScreen.kt** - Recipe detail view: Swipeable image carousel with HorizontalPager (240dp) showing all images from mediaPaths with page indicator dots when 2+ images, servings dropdown with auto-scaling, cook mode (checkable ingredients/instructions with Select All/Deselect All buttons in section headers, timer, keep awake), long-press ingredient for substitution lookup, tabbed instruction sections, tags, notes, favorite/edit/delete actions, Coil AsyncImage
