...
- **MealPlanningScreen.kt** - Meal planning list: Card-based list with search, duplicate/delete dialogs, shows all recipes and tags, enhanced recipe cards with servings/time/tags, action buttons (Edit, Generate List) inline with date range, full-screen recipe picker grid, auto-naming from dates
- **AddEditMealPlanScreen.kt** - Meal plan add/edit form: Name field, optional start/end date selection with Material3 AppDatePickerDialog, tabbed recipe picker (Import tab with URL/PDF/Photo options as default, Existing tab with 2-column grid and search), notes field, auto-save on back navigation, auto-naming from selected dates
